...
get_ls_unit <- function(unit, ppm = 5, abs_floor = 0.006) {
        mass_val <- tryCatch({
                mi <- getmass(unit)
                if (is.list(mi)) {
                        if (!is.null(mi$mass)) as.numeric(mi$mass)
                        else if (length(mi) >= 1) as.numeric(mi[[1]])
                        else NA_real_
                } else as.numeric(mi)[1]
        ...
        for (k in k_values) {
                ...
                if (!is.null(trend)) {
                        if (trend == "increasing") {
                                ...
                                good <- rep(TRUE, length(from_vec))
                        }
                        from_vec <- from_vec[good]
                        to_vec   <- to_vec[good]
                }
...
# ---------------------------------------------------------
build_graph <- function(indexes, mz, ls_unit, edges, rttol, trend, peaks2keep) {
...
strict_rt_filter <- function(df, trend = NULL) {
        ...
        for (sid in sids) {
                ...
                if (length(bad)) {
                        drop_idx <- comp$series_id.index[bad + 1]
                        df$series_id[df$series_id.index %in% drop_idx] <- NA
                }
        }
        df
}
...
find_homologues <- function(
        ...
        if (verbose) {
                message(sprintf("Peaks: %d | Candidate edges: %d", length(mz_sorted), nrow(edges)))
